...
# Style CSS (Fond noir + Glassmorphism + Rouge #c1002c)
st.markdown(f"""
    <style>
    ...
    h1 {{ color: #ffffff !important; font-family: 'Inter', sans-serif !important; text-align: center; margin-bottom: 30px; }}
    ...
    /* Textes */
    .card-source {{ color: #c1002c !important; font-size: 10px; font-weight: 700; text-transform: uppercase; margin-bottom: 8px; letter-spacing: 0.5px; }}
    .card-title {{ font-size: 14px; font-weight: 700; line-height: 1.4; height: 40px; overflow: hidden; margin-bottom: 8px; }}
...
if col1.button("ARTICLES PRESSE", type="primary" if st.session_state.view == 'Articles' else "secondary", use_container_width=True):
    st.session_state.view = 'Articles'; st.rerun()
if col2.button("VIDEOS YOUTUBE", type="primary" if st.session_state.view == 'Videos' else "secondary", use_container_width=True):
...
if not df.empty:
    ...
    for idx, row in df.reset_index().iterrows():
        with cols[idx % 4]:
            summary_div = f'<div class="card-summary">{row["summary"]}</div>' if row["summary"] else '<div style="height:38px;"></div>'
            st.markdown(f"""
                <div class="card">
